...
##set the types
  ...
  transform_y=getYTransform(pow = 1,    ## transformations raise to the powers indicated here.  Default is 1 but can add others
                            n_random=10  ## number of random transformations, also used in stopping critera
                            )
  ## set the flags
...
testIterative<-function(example, prop_na=0.2, seed=42){
  ...
  runExample<-function(example){
   ...
   # vars_all = analysis$select(datas,  flags, transform_y, phens = dh$pheno()$all)
    ...
    ggps1=plotEval(eval1,legend=T, grid1=c("subpheno","pheno"), grid0=c("measure","cv_full"),linetype="data", ##"full_model"
                   shape_color=c("data","transf"),sep_by=c("beam"), showranges=T,text="variable",logy=F,
...
testIterative("binomial", prop_na=0.2, seed=42)
```