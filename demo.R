...
##can have area for binomial or multinomial
options("fspls.types"= fromJSON('{"gaussian": ["correlation","rms"],"binomial":["AUC","max_diff","max_diff_x"],"multinomial":["AUC","area","max_diff","max_diff_x"],"ordinal" : "AUC_all"}'))
...
flags = list(pthresh = 0.05, max=20,nrep=1,batch=0,topn=50,beam=1,all_v_all=F,  project=T,  stop_y="rand",x_transform=T,
             transform_y = toJSON(transform_y), useoffset=T,useglmnet=T,loadPV=T, angles_only=T, get_plots=T
)
...
comb_plot=attr(vars_all,"plots")
  ...
  ggps1=.plotEval2(eval1,legend=T, grid1=c("subpheno","pheno","beam"), grid0="measure",linetype="beam", ##"full_model"
  ...
  ggps1$`CV= TRUE FULL= TRUE`
  ggps1$`CV= TRUE FULL= FALSE`
  pdf("output.pdf")
  lapply(ggps1, print)
  dev.off()
  
  if(FALSE){ ## show cumulative plots for calibration
    ##extract all predictions
    predictions0 =list(
      dh$extractPredictions(all_modelsh, CV = F, liab=F)
    )
    names(predictions0)=nme
      
    families=dh$data$family[[1]]
